          ...
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in" style={{ animationDelay: '0.3s' }}>
              ...
              <Button asChild variant="outline" size="lg">
                <Link
                  to="/category/tech"
                  data-semtag-id="home.hero.tech"
                  data-semtag-role="navigation"
                  data-semtag-target="category.tech"
                >
                  Explore Tech
                </Link>
              </Button>
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <Button asChild variant="ghost" className="hidden sm:inline-flex">
                <Link
                  ...
                  data-semtag-id="home.featured.viewall"
                  data-semtag-role="navigation"
                  data-semtag-target="products.featured"
                >
                  View all
            ...
            {categories.map((category) => {
              ...
              return (
                <Link
                  ...
                  data-semtag-id={`home.categories.item.${category.id}`}
                  data-semtag-role="navigation"
                  data-semtag-target={`category.${category.id}`}
                >
                  ...
                  <div className="flex items-center text-sm text-muted-foreground">
                    <span
                      data-semtag-id={`home.categories.item.${category.id}.count`}
                      data-semtag-role="observable"
                      data-semtag-state="category.productCount"
                    >
            ...
            <div className="container mx-auto px-4">
              <div className="flex items-center justify-between mb-8">
                ...
                <Button asChild variant="ghost">
                  <Link
                    to={`/category/${category.id}`}
                    data-semtag-id={`home.category.${category.id}.viewall`}
                    data-semtag-role="navigation"
                    data-semtag-target={`category.${category.id}`}
                  >
                    View all
              ...
              <div
                ...
                data-semtag-id={`home.category.${category.id}`}
                data-semtag-role="collection"
              >
                {categoryProducts.map((product) => (
                  <ProductCard
                    key={product.id}
                    product={product}
                    collectionId={`home.category.${category.id}`}
                  />
                ))}
              </div>
            </div>
